...
const AgregarEmpleado = () => {
  ...
  const handleDuiChange = (e) => {
    let duiValue = e.target.value.replace(/[^\d]/g, ""); // Elimina caracteres no numéricos
    if (duiValue.length > 9) {
      duiValue = duiValue.slice(0, 9); // Limita a 9 caracteres
    }
    if (duiValue.length > 8) {
      duiValue = duiValue.slice(0, 8) + "-" + duiValue.slice(8); // Inserta un guion después del octavo dígito
    }
    setDui(duiValue);
  };

  const handleTelefonoChange = (e) => {
    let telefonoValue = e.target.value.replace(/[^\d]/g, ""); // Elimina caracteres no numéricos
    if (telefonoValue.length > 8) {
      telefonoValue = telefonoValue.slice(0, 8); // Limita a 8 caracteres
    }
    if (telefonoValue.length > 4) {
      telefonoValue = telefonoValue.slice(0, 4) + "-" + telefonoValue.slice(4); // Inserta un guion después del cuarto dígito
    }
